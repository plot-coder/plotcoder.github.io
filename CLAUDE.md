...
- **Project data** — the boards, their order, the premise — lives on the project record in `src/board/project.js`; reminders keep their own `plotcoder.*` key. One `plotcoder.board.<id>` per board.
- **New editable text** uses `src/EditableText.tsx`. Do not write another `contentEditable`.
- **The account mirror is a mirror, not a lock.** `src/board/account.ts` keeps a signed-in writer's project on Supabase; the rules are in `src/board/sync.js` and are pure — change them there, with a test. Signed out, none of it runs; tests and the e2e suite never touch the network. Never put a secret in the client: the URL and publishable key are meant to ship, and row-level security is the wall.
