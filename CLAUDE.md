...
- **A thread (R60) is a record beside the fold and the setup arrow, not instead of them.** `state.threads` is the writer's named string through cards with either end open; the fold and the arrow still draw a plant between two scenes that exist. The reading asks about a thread only from its loose ends. Two records for one idea, on Robert's word — the combine log has the rule if they ever contradict: fold the thread's tied ends into a corner and an arrow, not the other way.
- **One question decides whether a card is in the film:** `inStory(note)` in the kernel — not a version behind another (R65) and not set aside (R66). Anything that counts, orders, paginates, exports, tidies or reads the story asks it, never `alternativeOf` or `aside` directly. An **open** card (R59) is a different thing on purpose: it is in the film. And one undecided field is not an open card: a place, a when and a change line each have an open of their own (R61, R67), and the card's other questions stand.
...
- **The writer's guide is part of the surface.** `public/writers.html` says how a person uses the app (R63). A change to a gesture, a sheet, a button or a claim a card can make changes that page in the same pull request; the agent's on-ramp and guide already work this way.
...
- **Plan, ask, build, test — in that order, for every item** (Robert, 2026-09-20). First a plan. Then ask, in writing, whether it is the best we can do: for anything a person will see that means a mockup on the app's own paper beside what ships, and Robert's word; tools and plumbing skip the mockup, not the question — name the alternatives and why this one wins, and change the plan if the honest answer is no. Then build. Then test, and look at it in the app when a person can see it. `docs/to-do-round-twenty-two.md` is the pattern worked through a whole round's findings: every entry accounted for, every material one an item with those four steps; `docs/to-do-round-twenty-three.md` is the list in work today.
...
- **Blind runs** are how the agent surface gets tested: a fresh agent, the on-ramp and a treatment, nothing else, and a friction log we work through. The rules and the rounds are in `blind-runs/`. **The open queue is the latest round's report** — today `blind-runs/round-twenty-three-report.md`; its head says what was fixed from it and what was decided against. Read it before starting on the agent surface. A round works a marked test account, cleared with `scripts/wipe-test-account.mjs` (R44) — prefer `--empty` between rounds so the credentials in the prompt stay put.
