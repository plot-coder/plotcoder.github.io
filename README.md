...
Steps 1 to 3 are built. Step 4 has its first slice: the runs between beats and five kinds of question, in the Reminders modal and as a `read_wall` tool. Step 5 is not started. See "What is built and what is left" in the requirements.
...
- **The MCP server**, `scripts/plotcoder-mcp.mjs`, wired for Cursor in `.cursor/mcp.json` and for Claude Code in `.mcp.json`. Sixteen tools: `list_board`, `read_wall`, `set_logline`, `set_target`, and the card, group, and arrow verbs. If the dev app is open, a tool call lands on the wall within a second; if not, it edits the board file and the wall catches up on the next load.
