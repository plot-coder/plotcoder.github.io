...
- **The MCP server**, `scripts/plotcoder-mcp.mjs`, wired for Cursor in `.cursor/mcp.json` and for Claude Code in `.mcp.json`. Forty-six tools: `list_board`, `read_wall`, `read_pages`, `page_count`, `list_workflows`, `segment_brief`, `undo`, `organize`, `apply_template`, `export_fountain`, `import_fountain`, `export_fdx`, `import_fdx`, `write_scene`; the project's `list_boards`, `open_board`, `new_board`, `rename_board`, `delete_board`, `set_premise`, `rename_project` and the reminders; `set_logline`, `set_target`, and the card, cast, place, group, and arrow verbs. If the dev app is open, a tool call lands on the wall within a second; if not, it edits the board file and the wall catches up on the next load.
