...
- **As built:**
  ...
  - **The way back (Roadmap 2, item 1, built 2026-09-13):** the identity became the email, so Supabase's own recovery works — Forgotten? at the door for an address the account knows, a reset link, and a "new password" form on the wordmark's sheet when the writer arrives on it. The `names` table became `writers` keyed by email; sharing is by email; People shows emails; the edge function claims an address and changes it. **Dashboard step, not reachable from the code:** the reset link lands on the project's Site URL unless `https://plotcoder.com/**` and `http://localhost:5173/**` are in Authentication → URL configuration → Redirect URLs; and reset emails go through Supabase's built-in sender until an SMTP sender is set, which limits them to a few an hour.
...
- **Status:** **built** 2026-09-13; see R4. What remains here is the dashboard step (redirect URLs) and the move to a project of its own before a second writer.
