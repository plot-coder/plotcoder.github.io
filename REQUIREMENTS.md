...
- **Notes:** One project per writer for now — the mirror meets whichever project the account holds. Several projects per writer, two people on one board, and Realtime wait. **To finish the door, in the Supabase dashboard (not reachable from the code):** add `https://plotcoder.com/**` and `http://localhost:5173/**` under Authentication → URL configuration → Redirect URLs, or the emailed link lands on the Bee Healers site URL. **Before a second writer signs in**, move to a project of PlotCoder's own (D27): the two tables copy whole.
...
- **The account mirror (R4)** is `src/board/account.ts`: a thin network layer over `src/board/sync.js` (pure, tested — meeting the account, push outcomes, open outcomes, the conflict rule). The client is `src/supabase.ts`, made on first use so tests never touch it; `VITE_SUPABASE_URL` and `VITE_SUPABASE_KEY` in a `.env.local` point at another project. Auth session and sync bookkeeping live under `sb-plotcoder-*` keys, outside the `plotcoder.` prefix, so Save/Open never carry them. The store grew `boardState`, `replaceBoard` (an undo step on the open board), `adoptProject` and `isSeedProject` for the mirror. Tables are `public.plotcoder_projects` and `public.plotcoder_boards` on the `beehealers` project, RLS by owner; the migration is `plotcoder_projects_and_boards` in that project's history. The magic link needs the app's origins in the dashboard's redirect allow-list.
