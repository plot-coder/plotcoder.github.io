...
- **As built:**
  ...
  - **Agent:** `update_character` with the same five lines (thirty tools now); `list_board`'s cast lines say "page: looks, wants" or "page: empty", so an agent can see who is a brief and who is still a name. The skill says to ask the writer before inventing looks or a voice.
...
- **Notes:** Read the wall's *unwritten* question keeps asking about a beat whose change line is still the prompt, which is right: the prompt is not an answer. A structure could later be a writer's own, saved from a wall's beats; not built.
