...
- **Since then:** the cast (R29) added "a character who disappears" and "a person on no card". Still not yet: a setup with no payoff, which needs typed arrows (P16). The board cannot read what it does not hold.
...
| 4. Read the wall | R22 | **built in two slices** — runs between beats and seven kinds of question, including a character who disappears (R29); typed arrows still to come | `read_wall` |
