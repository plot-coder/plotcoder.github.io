...
**2026-09-22: 0.1.51 is released** (npm, the pin; the function redeployed by Robert's hand — the session's permission layer declines the deploy flag, as it does the tag). **Robert set three goals, recorded as D28, and the plan is `docs/plan.md`:** a complete script, a person jumping in after the agent has worked, and projects past an agent's context window worked hands-off — six passes with a claim board, worked by many sessions at once. **Start there, not here.** Still waiting on Robert's hand: D4 (the connector's instructions, a cold start), R72 (the door's sign-in), a mark on an unlinked card.

**2026-09-21, later: round twenty-four's list is built** — D1, D2, D3 (R73) and N1 to N15 on Robert's "come up with a good plan and then execute", 713 unit tests, e2e ten of ten, `npm run build` clean, looked at in the app (the two-arrow pair, the unlinked card left by the tidy). **Uncommitted on the branch `claude/docs-review-pass-e9ef7e`.** Next: commit and open the pull request; release (`npm version patch`, tag, wait for npm, bump the pin in `supabase/functions/mcp/index.ts`, redeploy the function, check the live door); then **D4**, the cold-start test of whether the desktop connector delivers the server's instructions (Robert at the screen); then **R72**, the door's sign-in, on his word; and one thing to draw: a mark on an unlinked card on the wall.

**2026-09-21, night: round twenty-four has run** — the connector alone, "Doyle's", 0.1.50: fifty-four entries, `blind-runs/round-twenty-four-report.md`, verbatim under a head that says what it measured. Nothing fixed yet. **The working list is written: `docs/to-do-round-twenty-four.md`** — every entry accounted for; four decisions for Robert (D1 a thread through a version behind, D2 a follows and a setup arrow on one pair, D3 an unplaced card out of the order, D4 the instructions and the desktop connector, a cold-start test by hand), fifteen fixes a session can make (N1 to N15), four left, the rest decided against. Nothing built. The head names the material ones: a thread through a version behind drops the card silently (17); `choose_version` carries the front card's setup arrow onto the version that steps forward (35); `set_order` skips an arrow a setup arrow holds and does not say (26); a stock "version went with it" line on an unrelated delete (33); an aside card quiets the cast check and one reply contradicts itself (16, 22); the tidy seats an unwired card and the runs count it (21, 28, 34); the default export is kept where `empty_account` clears it (10); and **the server's `instructions` did not reach the agent through the desktop connector** (5) — the door sends them; find out where they go. R72 (the door's sign-in) is planned and drawn, waiting on Robert's word.

**2026-09-21, afternoon:** round twenty-four is pasted into a fresh no-folder session and stopped at the way in: the session had no PlotCoder connector (not enabled for it); its entry 1 is that the absence cannot be told apart from a silent failure. The stale `plotcoder-board` stdio server was taken out of `~/.claude.json` (backup `~/.claude/backups/claude.json.before-round-24`). **Robert's new need, R72 — a sign-in for the door:** planned in `REQUIREMENTS.md` and drawn in `docs/mockups/r72-the-sign-in.html`, waiting on his word. Not built. The round runs on the header as designed; R72 is for after it.


**For whoever picks this up (2026-09-21):** everything is merged — `main` is clean, no pull request is open, and `main`, the registry, plotcoder.com and the hosted door all carry **0.1.50**. Nothing is half-built. The one thing to do next is **run round twenty-four**: paste `blind-runs/prompt.md` from below its first divider into a fresh desktop session with no folder, with the PlotCoder connector turned on for that session only (it is signed in as the test account; turn it off everywhere else). The writer's answers and the directions are above the divider; the owner relays them, or the cueing session does. "The Tuner" from twenty-three is still on the test account, and the round's agent saves and empties it itself. When the round stops, file its report verbatim as `blind-runs/round-twenty-four-report.md` under a head, and work it as `docs/to-do-round-twenty-three.md` was worked: plan, ask, build, test. A release is: merge, `npm version patch` and push the tag **once** from a current `main`, wait for `npm view plotcoder-board dist-tags.latest`, bump the pin in `supabase/functions/mcp/index.ts`, `supabase functions deploy mcp --no-verify-jwt --use-api --project-ref kmpahjsggbleygsnuwug`, check the live door on a throwaway project, delete it. A migration is `supabase db push --dry-run`, then `supabase db push`. Two calls made for the owner that are his to overrule: an open title prints "Untitled", and the camera check leaves out "hears", "sees" and "notices".
...
1. ~~**Land the round's fixes**~~ **Landed 2026-09-20, night:** #139 merged, released as 0.1.47, the door's pin bumped and the function redeployed; the live door checked. What it was: The merge and the migration-style writes are run by hand (a session's permission layer declines them); the rest a session can do. **Do not click a release block twice:** 0.1.45 and 0.1.46 are one release run from two places.
2. **Built 2026-09-20 on "Recommendations", merged as #142 on the owner's "go" and released as 0.1.48:** R69 (the cast line's own open), R70 ("Not decided yet, about the film", under the logline) and R71 (a proposed turn, dashed, kept or struck), with the checklist's five questions and the words for how a plant pays off; 676 unit tests, e2e ten of ten. What was drawn: `docs/mockups/round-twenty-three-i-dont-know.html` — seven items, each as it ships beside what it could be. Recommended: build the cast line's own open (1) and "Not decided yet, about the film", a short list under the logline (2, to be seen in the app before it ships); nothing on the card for lengths (3); words for how a plant pays off — a `[[note]]` on an unwritten scene is already a safe home, checked (4); a proposed turn drawn with a dashed bar, held less firmly (5); leave a person's one open line (6); five questions added to the asking's checklist (7). What it was: **the round's real finding:** a writer's "I don't know" has a home for a place, a when, a change line and a named person, and none for **who is in a scene when nobody can be named, a when true of the whole film, a length, the acts, plants not yet known** (entries 13 to 16). F8 was left twice; this is its third asking and the wall contradicted the writer. With it on one page: how a plant pays off when the change line is the writer's own words (17), a proposed turn (32, F5's second asking), one open string per person (18), and the asking's checklist, which has no question for a change line, a cut, two versions, a maybe or a person's open (9).
3. **Decide what "the session's first reading" means.** The count-and-point tail (call 2) is unreachable by an agent who follows the on-ramp, whose first calls include `read_wall` — in twenty-three, of someone else's stale wall. Options: the first reading *of this project* (reset on `new_project` and `open_project`); or take `read_wall` out of the first calls when the account's working project is not the agent's own; or let it go and always quote.
4. **The smaller open entries**, listed in the report's head: `of` on `create_note` (20), maybes in the pages' scene note (54), a reply saying what a cut or a reorder did to the runs (31, 50, 52, 72), the on-ramp and the guide for someone holding a connector (1, 2, 6, 7).
5. C1's OAuth proposal, R64's weekly agent, a real undo through the hosted door: as before.

## The queue before it (2026-09-20, after round twenty-two)

**Before round twenty-three (2026-09-20, late; the suite is 32 files and 642 tests):** B1 is built on its branch — a session for the hosted door, as **one table and no secret** (R48's "A session"; `src/board/agentSession.js`). **The migration is applied** (2026-09-20, `supabase db push` from the linked CLI): `public.agent_sessions` is on the plotcoder project, and the store B1 ships was round-tripped against it as the test account — a new id loads fresh, a save loads back, signed out the row is invisible. **The migration history was repaired the same hour and now matches the repo, so `supabase db push` is how a migration goes in from here:** the connector had recorded the first two migrations under its own timestamps (`20260913144532`, `20260913155704`; same SQL, checked by diff) and the two `questions` migrations had gone in as raw SQL and were in no history; the first pair was marked reverted and the repo's four marked applied, which touches only the history table. `supabase/.temp` holds the link and is not committed. **Landed 2026-09-20:** #137 merged and **released as 0.1.46** (0.1.45 is the same code); the pin in `supabase/functions/mcp/index.ts` says 0.1.46. **The function is redeployed and the live door checked (2026-09-20):** `initialize` answers 0.1.46 with an `mcp-session-id`; with the id, the first write counts and points at `read_wall` and says its advice, the second does not say it again, and after a `read_wall` the tail stops counting; with no id the door behaves as patched; `Tomás?` lands as a maybe; `undo` says it keeps no trail. Checked as the test account on a throwaway project, deleted after. Not the Worker: it forwards every header. Check with one `initialize` that the reply carries an `mcp-session-id` header, then two `create_note` calls carrying it: the first says "pass color to choose" and the second does not. **Still unwatched, and the round's to watch:** that the desktop connector sends the id back — the protocol requires it; if it does not, the door behaves as it did, patched. Then the round: **cued 2026-09-20** — `blind-runs/prompt.md` is round twenty-three's, on `round-twenty-three-idea.md`, "The Tuner"; twenty-two's prompt is archived. It wires the connector and nothing else, with no fallback, and its head says what it measures. It must not be run before the release after 0.1.44 is on npm and the door. H8, H9 and H10 are built on a yes to all three (`docs/mockups/round-twenty-two-last-three.html`; H9 is **R68**, "Tomás?" on the cast line), so the round's notes — which have someone who may or may not be in a room — now measure a field, not its absence. The suite is 33 files and 654 tests. C1's probe needs a person at the connector screen and fits the round's wiring.

**Where that list stood (2026-09-20, evening):** twenty-six of its thirty-one items are built and tested (the other five: A2's connector, B1, C1, C2, and F8 left on purpose), the seven designs among them on Robert's word (`docs/mockups/round-twenty-two-designs.html`). Released as 0.1.44 and deployed: the round's fixes, R66, R67. **Merged and not yet released:** everything after — `who_is_here`, `set_order`, the one view of what is undecided, the short read, no tool tidying on its own, "a feature" as the writer's word, the person's open line, the camera's mark in Pages. **Left:** B1, a session for the hosted door (wants Robert's yes: a table and a secret); C1, the connector screen's probe and an OAuth proposal; C2, round twenty-three through the connector alone — and that round should hand an agent a writer who cuts a scene and keeps it, says "I don't know what changes", says "it is a feature", and has something undecided about a person.
...
3. **R64's weekly agent, by hand** (the other session's note): a scheduled
   ...
   questions migration showed in the migration history, because both went in
   through the Management API as SQL (repaired 2026-09-20: the history now
   matches the repo). Three
