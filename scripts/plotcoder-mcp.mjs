#!/usr/bin/env node
// The PlotCoder MCP server's front door. The server itself is
// plotcoder-mcp-server.mjs; this file only checks that it can start.
//
...
const installed = fs.existsSync(path.join(root, "node_modules", "@modelcontextprotocol", "sdk", "package.json"));

if (installed && process.env.PLOTCODER_PRETEND_NOT_INSTALLED !== "1") {
  await import("./plotcoder-mcp-server.mjs");
} else {
