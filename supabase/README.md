...
- `functions/mcp/` — the hosted MCP door (R48; the handover's call 3, Robert's word 2026-09-19: host it on Supabase). The published package through `npm:`, the SDK's fetch-based transport, one writer per request signed in from the request's Basic header, no disk. Deployed with `supabase functions deploy mcp --no-verify-jwt --use-api --project-ref kmpahjsggbleygsnuwug` (`--use-api` bundles server-side, so Docker is not needed). The function's own address is `https://kmpahjsggbleygsnuwug.supabase.co/functions/v1/mcp`; the name in front of it is `https://mcp.plotcoder.com`, a Cloudflare Worker in `cloudflare/mcp-door`. Verified 2026-09-19: initialize answers, `list_projects` reads the signed-in account, a wrong password is refused by the tool. It carries the package version pinned in `index.ts`; bump it with each release the door should carry.
